<template>
  <div>
    <div class="d-flex justify-content-between mb-3">
        <p class="fs-3 fw-bold ms-3">Crie uma conta</p>
        <span
        @click="close"
        class="fs-2"
      ...
      <form @submit.prevent="handleSubmit" class="d-flex flex-column gap-5">
        <div class="d-flex flex-column gap-4">
            <div class="d-flex flex-column input-group gap-2">
                ...
                :class="{'border border-2 border-danger' : !!state.name.errorMessage}"
  ...
  setup(){
    ...
    async function handleSubmit(){
      try {
        ...
        const {data, errors, messageSuccess} = await services.auth.register({
